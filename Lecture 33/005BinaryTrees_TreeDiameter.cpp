...
// 	int Z = left.height + right.height + 2;

// 	p.height = 1 + max(left.height, right.height);
// 	p.diameter = max({left.diameter, right.diameter, Z});
...
pair<int, int> computeDiameterEfficient(TreeNode* root) {
	...
	int Z = left.first + right.first + 2;
...
int main() {
	...
	cout << computeDiameterEfficient(root).second << endl;
