...
int minSteps(int n) {
	...
	// recursively, compute the min. steps required to reduce n-1 to 1
	...
	int y = INT_MAX;
	if (n % 2 == 0) {
		// recursively, compute the min. steps required to reduce n/2 to 1
		y = minSteps(n / 2);
	}
	...
	int z = INT_MAX;
	if (n % 3 == 0) {
		// recursively, compute the min. steps required to reduce n/3 to 1
		z = minSteps(n / 3);
	}
...
int minStepsBottomUp(int n) {
	...
	for (int i = 2; i <= n; i++) {
		...
		// dp[i] = f(i) : min. number of steps required to reduce i to 1
