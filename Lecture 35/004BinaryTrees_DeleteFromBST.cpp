...
TreeNode* deleteFromBST(TreeNode* root, int key) {
	...
	if (key < root->val) {
		// recursively, delete the key, if present, from the LST
		root->left = deleteFromBST(root->left, key);
	} else if (key > root->val) {
		// recursively, delete the key, if present, from the RST
		...
		if (root->left == NULL and root->right == NULL) {
			...
			// swap(leftMax->val, root->val);
			// root->left = deleteFromBST(root->left, key);

			TreeNode* rightMin = findMinimum(root->right);
			swap(rightMin->val, root->val);
			root->right = deleteFromBST(root->right, key);
		}
	}
