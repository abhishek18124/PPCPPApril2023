...
int editDistance(string s1, string s2, int m, int n, int i, int j) {
	...
	int x = editDistance(s1, s2, m, n, i + 1, j);

	// 2. substitute
	int y = editDistance(s1, s2, m, n, i + 1, j + 1);

	// 3. insert

	int z = editDistance(s1, s2, m, n, i, j + 1);

	return min({1 + x, (int)(s1[i] != s2[j]) + y, 1 + z});
...
int editDistanceBottomUp(string s1, string s2, int m, int n) {
	...
	for (int i = m - 1; i >= 0; i--) {
		...
		for (int j = n - 1; j >= 0; j--) {
			...
			dp[i][j] = min({1 + x, (int)(s1[i] != s2[j]) + y, 1 + z});
		}
